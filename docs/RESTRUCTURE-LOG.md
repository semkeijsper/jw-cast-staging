...
**Rework after hardware test (exclusive mode):** pause-only was rejected — receiver ignored `LoadRequest.currentTime` (cast started at 0) and two live players kept conflicting. Now: `castActive` (`isConnecting || isCasting`) v-ifs the `<video>` out entirely and shows a poster placeholder + device name; local Plyr is destroyed on cast start; the start position is enforced by an explicit `RemotePlayerController.seek()` once the receiver reports media loaded (`pendingStartTime` in `useCast`); when the cast ends with the dialog open, the local player rebuilds at the last cast position. Non-cast paths regression-tested green (phase-1 + phase-3 suites).

- [ ] **Manual re-check (Chromecast hardware):** play locally → cast → player area becomes placeholder, cast starts at the local position; stop cast with dialog open → local player returns at the cast position.
